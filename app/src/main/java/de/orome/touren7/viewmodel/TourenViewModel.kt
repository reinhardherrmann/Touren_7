...
class TourenViewModel(private val tourenRepository: TourenRepository): ViewModel(), Observable {
    ...
    fun cancelInsertTour() {
        clearFormValues()
        statusMessage.value = Event("Eingabe beendet.")
    }
    ...
    fun updateTour(tour: Tour) = viewModelScope.launch {
        val rowCount = tourenRepository.updateTour(tour)

        if (rowCount > 0){
            statusMessage.value = Event("Tour ${tour.tourNummer} aktualisiert")
        } else{
